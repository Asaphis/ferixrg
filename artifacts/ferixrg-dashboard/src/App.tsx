...
function DashboardApp() {
  ...
  function Overview() {
    ...
    return <div className="page">
      <div className="page-header"><div><span className="eyebrow">Signal desk · Tuesday, 18 June</span><h1>Make the next store decision obvious.</h1><p>FerixRG turns storefront evidence into a short, reviewable path from observation to improvement.</p></div><div className="header-actions"><button className="btn" onClick={() => navigate("stores")}><Plus size={15} /> Add store</button><button className="btn primary" onClick={() => navigate("analysis")}><Play size={15} /> Run analysis</button></div></div>
      <div className="grid metric-grid">
        <div className="metric" style={{ "--metric-color": "var(--lime)" } as CSSProperties}><div className="metric-top"><span>Connected stores</span><Store size={15} /></div><div className="metric-value">{stores.filter((store) => store.status === "Connected").length}</div><div className="metric-note good">All sources responding</div></div>
        <div className="metric" style={{ "--metric-color": "var(--cyan)" } as CSSProperties}><div className="metric-top"><span>Average health</span><Activity size={15} /></div><div className="metric-value">{average}<small style={{ fontSize: 13, color: "var(--text-dim)" }}>/100</small></div><div className="metric-note good">+4.8 pts this week</div></div>
        <div className="metric" style={{ "--metric-color": "var(--red)" } as CSSProperties}><div className="metric-top"><span>Open issues</span><AlertCircle size={15} /></div><div className="metric-value">{openIssues}</div><div className="metric-note warn">3 need a decision today</div></div>
        <div className="metric" style={{ "--metric-color": "var(--purple)" } as CSSProperties}><div className="metric-top"><span>Reports ready</span><FileBarChart size={15} /></div><div className="metric-value">{reports.length}</div><div className="metric-note">Last run 18 minutes ago</div></div>
      </div>
      <div className="grid two-col">
        <section className="panel"><div className="panel-heading"><div><span className="eyebrow">Primary signal</span><h2>Storefront health</h2><p>Scores combine the latest evidence across six decision areas.</p></div><button className="btn ghost" onClick={() => navigate("reports")}>View reports <ArrowRight size={14} /></button></div>
          <div className="score-layout"><div className="score-ring"><div className="score-ring-inner"><strong>{average}</strong><span>overall score</span><small>↑ 4.8 pts</small></div></div><div className="bars">{[["Design", 88, "var(--purple)"], ["UX", 79, "var(--cyan)"], ["Mobile", 76, "var(--orange)"], ["Accessibility", 91, "var(--lime)"], ["SEO", 84, "var(--cyan)"], ["Conversion", 73, "var(--red)"]].map(([label, score, color]) => <div className="bar-row" key={String(label)}><span>{label}</span><div className="bar"><i style={{ width: `${Number(score)}%`, "--bar-color": color } as CSSProperties} /></div><b>{score}</b></div>)}</div></div>
        </section>
        <section className="panel"><div className="panel-heading"><div><span className="eyebrow">Shortcuts</span><h2>Move the work forward</h2></div></div><div className="action-grid">
          <button className="action-card" onClick={() => navigate("analysis")}><ScanIcon /><span>Analyze a storefront<small>Start with URL or store evidence</small></span></button>
          <button className="action-card" onClick={() => navigate("redesign")}><Sparkles /><span>Generate redesign<small>Turn findings into a proposal</small></span></button>
          <button className="action-card" onClick={() => navigate("issues")}><ShieldCheck /><span>Review priorities<small>{openIssues} open findings to triage</small></span></button>
          <button className="action-card" onClick={() => navigate("editor")}><Layers3 /><span>Open Design Studio<small>Continue the working draft</small></span></button>
        </div></section>
      </div>
      <div className="grid two-col" style={{ marginTop: 16 }}>
        <section className="panel"><div className="panel-heading"><div><span className="eyebrow">Registry</span><h2>Active stores</h2></div><button className="btn ghost" onClick={() => navigate("stores")}>See all <ArrowRight size={14} /></button></div><div className="grid" style={{ gap: 9 }}>{stores.map((store) => <button className="store-card action-card" key={store.id} onClick={() => { setActiveStoreId(store.id); setSelectedStoreDetail(store.id); navigate("stores"); }}><div className="store-mark">{initials(store.name)}</div><div className="store-info"><strong>{store.name}</strong><span>{store.domain} · {store.platform}</span></div><div className="health-number">{store.health || "—"}<small>/100</small></div><ChevronRight size={15} /></button>)}</div></section>
        <section className="panel"><div className="panel-heading"><div><span className="eyebrow">Live feed</span><h2>Recent activity</h2></div><button className="btn ghost" onClick={() => pushToast("Activity is up to date")}>Refresh <RefreshCw size={13} /></button></div><div className="activity-list">{activity.map(([event, detail], index) => <div className="activity-item" key={`${event}-${index}`}><i className="activity-dot" /><div><strong>{event}</strong><span>{detail}</span></div><ChevronRight size={14} color="var(--text-dim)" /></div>)}</div></section>
      </div>
    </div>;
  }
  function Stores() {
