...
describe("public URL inspection executor", () => {
  it("records only observed HTML metadata and accessibility-relevant document facts", async () => {
    ...
    await expect(inspectPublicUrl("https://shop.example")).resolves.toMatchObject({ statusCode: 200, title: "Store", language: "en", hasViewport: true, headingCount: 1, headings: [{ level: 1, text: "Store" }], imageCount: 1, imagesWithoutAlt: 1, linkCount: 1 });
  });
