  ...
  executePublicUrlToolRun: protectedProcedure.input(workspaceInput.extend({ toolRunId: z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    try {
      ...
      try {
        ...
        const resultSummary = { execution: running.toolId === "heading-structure-analyzer" ? "deterministic_heading_structure_inspection" : "deterministic_public_url_inspection", toolId: running.toolId, inspection, observedIssueIds: issueRecords.map(issue => issue.id) };
