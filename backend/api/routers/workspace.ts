...
export const workspaceRouter = router({
  ...
  componentBuilder: protectedProcedure.input(workspaceInput.extend({ toolRunId: z.number().int().positive().optional(), draftId: z.number().int().positive().optional(), message: z.string().trim().min(1).max(11_500), context: z.record(z.string(), z.string().max(500)).optional() })).mutation(async ({ ctx, input }) => {
    try {
      await requireWorkspaceAccess(ctx.user.id, input.workspaceId, "editor");
      if (input.toolRunId) {
        const run = await getWorkspaceToolRun(input.workspaceId, input.toolRunId);
        if (!run || run.toolId !== "component-builder") throw new TRPCError({ code: "BAD_REQUEST", message: "Component Builder must run from an active Component Builder tool run." });
      }
      if (input.draftId && !(await listWorkspaceDraftVersions(input.workspaceId, input.draftId))) throw new Error("workspace permission denied");
      const now = new Date();
      const utcDayStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
      const usedNeurons = await getWorkspaceAiNeuronUsageSince(input.workspaceId, utcDayStart);
      if (usedNeurons > CLOUDFLARE_FREE_DAILY_NEURON_LIMIT - CLOUDFLARE_MAX_REQUEST_NEURON_RESERVE) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "Component Builder has reached the protected free daily capacity for this workspace. It resets at 00:00 UTC." });
      const response = await runDesignCopilotThroughGateway({ message: `Create a reviewable component proposal only. Preserve the user’s manual component controls and do not claim that changes were applied, published, accessed, or inspected.\n\n${input.message}`, context: { ...(input.context ?? {}), operation: "Component Builder" } });
      const billedNeurons = Math.max(1, Math.ceil(response.neurons ?? 1));
