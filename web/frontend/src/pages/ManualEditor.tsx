...
type EditorStyle = {
  ...
  animation?: "None" | "Fade" | "Slide" | "Scale" | "Zoom";
  topDivider?: SectionDividerShape;
  bottomDivider?: SectionDividerShape;
  dividerHeight?: number;
};

type SectionDividerShape = "none" | "wave" | "curve" | "angle" | "u-shape" | "w-shape" | "custom-vector";
...
export default function ManualEditor({ context, mode, onModeChange, onBack, initialPanel = "Layers", initialInspectorTab = "Design" }: ManualEditorProps) {
  ...
  const resolveNodeStyle = (node: EditorNode) => ({ ...node.style, ...(node.responsive?.[device] ?? {}) });
  const selectedStyle = resolveNodeStyle(selected);
  ...
  return <section className="manual-editor" aria-label="FerixRG Manual Editor">
    <header className="manual-editor-topbar">
      <div className="manual-editor-brand"><button className="manual-icon-button" onClick={onBack} aria-label="Return to result"><ArrowLeft size={17} /></button><div><strong>FERIX<span>RG</span></strong><small>Manual Editor</small></div></div><span className="manual-mobile-saved"><i />Saved</span>
    ...
    <div className="manual-editor-body">
      ...
      <main className="manual-canvas-area">
        ...
        <div className="manual-canvas-stage">
          ...
          <article className={canvasClass} style={{ transform: `scale(${zoom / 100})` }}>
            ...
            {rootSections.filter(section => section.visible).map((section, index, sections) => <CanvasSection key={section.id} node={section} nextNode={sections[index + 1]} selectedId={selected.id} onSelect={setSelectedId} onDropElement={moveDraggedElement} children={children(section.id)} nestedChildren={children} resolveStyle={resolveNodeStyle} textStyle={selectedTextStyle} />)}
      ...
      <aside className={mobileInspector || mobileSheet === "properties" ? "manual-right-panel mobile-open" : "manual-right-panel"}><i className="manual-mobile-sheet-handle" aria-hidden="true" />
        ...
        <div className="manual-inspector-content">
          {inspectorTab === "Design" && <><InspectorSection title="Layout"><ControlRow label="Width" value={selectedStyle.width ?? "Auto"} control={<select value={selectedStyle.width ?? "Auto"} onChange={event => updateCurrentDeviceStyle({ width: event.target.value })}><option>Auto</option><option>Full width</option><option>Contained</option><option>Custom</option></select>} /><ControlRow label="Padding" value={`${selectedStyle.padding ?? 0}px`} control={<input type="range" min="0" max="96" value={selectedStyle.padding ?? 0} onChange={event => updateCurrentDeviceStyle({ padding: Number(event.target.value) })} />} /><ControlRow label="Radius" value={`${selectedStyle.radius ?? 0}px`} control={<input type="range" min="0" max="32" value={selectedStyle.radius ?? 0} onChange={event => updateCurrentDeviceStyle({ radius: Number(event.target.value) })} />} /></InspectorSection><InspectorSection title={`Appearance · ${device}`}><ControlRow label="Fill" value={selectedStyle.background ?? "None"} control={<input type="color" value={colorValue(selectedStyle.background, "#173b8f")} onChange={event => updateCurrentDeviceStyle({ background: event.target.value })} />} /><ControlRow label="Text" value={selectedStyle.color ?? "Default"} control={<input type="color" value={colorValue(selectedStyle.color, "#172039")} onChange={event => updateCurrentDeviceStyle({ color: event.target.value })} />} /><ControlRow label="Opacity" value={`${Math.round((selectedStyle.opacity ?? 1) * 100)}%`} control={<input type="range" min="20" max="100" value={(selectedStyle.opacity ?? 1) * 100} onChange={event => updateCurrentDeviceStyle({ opacity: Number(event.target.value) / 100 })} />} /></InspectorSection>{selected.kind === "section" && <InspectorSection title="Section shape divider"><ControlRow label="Top edge" value={selected.style.topDivider ?? "None"} control={<select value={selected.style.topDivider ?? "none"} onChange={event => updateSectionDivider("topDivider", event.target.value as SectionDividerShape)}><option value="none">None</option><option value="wave">Wave</option><option value="curve">Curve</option><option value="angle">Angle</option><option value="u-shape">U shape</option><option value="w-shape">W shape</option><option value="custom-vector">Custom vector</option></select>} /><ControlRow label="Bottom edge" value={selected.style.bottomDivider ?? "None"} control={<select value={selected.style.bottomDivider ?? "none"} onChange={event => updateSectionDivider("bottomDivider", event.target.value as SectionDividerShape)}><option value="none">None</option><option value="wave">Wave</option><option value="curve">Curve</option><option value="angle">Angle</option><option value="u-shape">U shape</option><option value="w-shape">W shape</option><option value="custom-vector">Custom vector</option></select>} /><ControlRow label="Shape height" value={`${selected.style.dividerHeight ?? 28}px`} control={<input type="range" min="12" max="96" value={selected.style.dividerHeight ?? 28} onChange={event => updateSelectedStyle({ dividerHeight: Number(event.target.value) })} />} /><p className="manual-property-note">Changes apply directly to this section. For a custom contour, use Pen or Pencil, then edit its nodes in Effects.</p></InspectorSection>}</>}
          {inspectorTab === "Layout" && <><InspectorSection title={`Layout · ${device}`}><ControlRow label="Display" value={selectedStyle.display ?? "Flex"} control={<select value={selectedStyle.display ?? "Flex"} onChange={event => updateCurrentDeviceStyle({ display: event.target.value as EditorStyle["display"] })}><option>Block</option><option>Flex</option><option>Grid</option><option>Inline</option><option>None</option></select>} /><ControlRow label="Direction" value={selectedStyle.direction ?? "Column"} control={<select value={selectedStyle.direction ?? "Column"} onChange={event => updateCurrentDeviceStyle({ direction: event.target.value as EditorStyle["direction"] })}><option>Row</option><option>Column</option><option>Row reverse</option><option>Column reverse</option></select>} /><ControlRow label="Align" value={selectedStyle.align ?? "Start"} control={<select value={selectedStyle.align ?? "Start"} onChange={event => updateCurrentDeviceStyle({ align: event.target.value as EditorStyle["align"] })}><option>Start</option><option>Center</option><option>End</option><option>Stretch</option></select>} /><ControlRow label="Gap" value={`${selectedStyle.gap ?? 16}px`} control={<input type="range" min="0" max="64" value={selectedStyle.gap ?? 16} onChange={event => updateCurrentDeviceStyle({ gap: Number(event.target.value) })} />} /></InspectorSection><InspectorSection title="Size & position"><ControlRow label="Width" value={selectedStyle.width ?? "Auto"} control={<select value={selectedStyle.width ?? "Auto"} onChange={event => updateCurrentDeviceStyle({ width: event.target.value })}><option>Auto</option><option>Full width</option><option>Contained</option><option>Custom</option></select>} /><ControlRow label="Position" value={selectedStyle.position ?? "Relative"} control={<select value={selectedStyle.position ?? "Relative"} onChange={event => updateCurrentDeviceStyle({ position: event.target.value as EditorStyle["position"] })}><option>Static</option><option>Relative</option><option>Absolute</option><option>Fixed</option><option>Sticky</option></select>} /><ControlRow label="Overflow" value={selectedStyle.overflow ?? "Visible"} control={<select value={selectedStyle.overflow ?? "Visible"} onChange={event => updateCurrentDeviceStyle({ overflow: event.target.value as EditorStyle["overflow"] })}><option>Visible</option><option>Hidden</option><option>Auto</option><option>Scroll</option></select>} /></InspectorSection><InspectorSection title="Spacing"><ControlRow label="Padding" value={`${selectedStyle.padding ?? 0}px`} control={<input type="range" min="0" max="96" value={selectedStyle.padding ?? 0} onChange={event => updateCurrentDeviceStyle({ padding: Number(event.target.value) })} />} /><ControlRow label="Margin" value={`${selectedStyle.margin ?? 0}px`} control={<input type="range" min="0" max="96" value={selectedStyle.margin ?? 0} onChange={event => updateCurrentDeviceStyle({ margin: Number(event.target.value) })} />} /></InspectorSection></>}
...
function CanvasSection({ node, nextNode, selectedId, onSelect, onDropElement, children, nestedChildren, resolveStyle, textStyle }: { node: EditorNode; nextNode?: EditorNode; selectedId: string; onSelect: (id: string) => void; onDropElement: (targetParentId: string) => void; children: EditorNode[]; nestedChildren: (id: string) => EditorNode[]; resolveStyle: (node: EditorNode) => EditorStyle; textStyle: { color?: string; fontSize: string; opacity: number } }) {
  const selected = selectedId === node.id;
  const sectionStyle = resolveStyle(node);
  const canvasStyle = (item: EditorNode, includeBackground = false): CSSProperties => {
    const style = resolveStyle(item);
    return {
      color: style.color,
      background: includeBackground ? style.background : undefined,
      ...
      margin: style.margin,
      gap: style.gap,
      boxShadow: style.shadow && style.shadow !== "None" ? style.shadow === "Large" ? "0 18px 32px rgba(29,47,83,.3)" : style.shadow === "Medium" ? "0 10px 22px rgba(29,47,83,.24)" : "0 5px 14px rgba(29,47,83,.17)" : undefined,
    };
  };
  const sectionCanvasStyle: CSSProperties = { background: sectionStyle.background, opacity: sectionStyle.opacity, borderRadius: sectionStyle.radius, padding: sectionStyle.padding, margin: sectionStyle.margin };
  const dividerColor = nextNode ? resolveStyle(nextNode).background ?? "#f8f6f1" : sectionStyle.background ?? "#172039";
  ...
  const topDivider = sectionStyle.topDivider && sectionStyle.topDivider !== "none" ? <SectionDivider edge="top" shape={sectionStyle.topDivider} height={sectionStyle.dividerHeight ?? 28} color={sectionStyle.background ?? "#e8ebf8"} /> : null;
  const bottomDivider = sectionStyle.bottomDivider && sectionStyle.bottomDivider !== "none" ? <SectionDivider edge="bottom" shape={sectionStyle.bottomDivider} height={sectionStyle.dividerHeight ?? 28} color={sectionStyle.background ?? dividerColor} /> : null;
  if (node.id === "header") return <section className={selected ? "manual-canvas-section canvas-header selected" : "manual-canvas-section canvas-header"} style={sectionCanvasStyle} onDragOver={event => event.preventDefault()} onDrop={() => onDropElement(node.id)} onClick={() => onSelect(node.id)}>{topDivider}<div className="canvas-header-inner"><button className={selectedId === headerLogo?.id ? "canvas-node selected" : "canvas-node"} onClick={event => { event.stopPropagation(); if (headerLogo) onSelect(headerLogo.id); }} style={headerLogo ? canvasStyle(headerLogo) : undefined}><strong>{headerLogo?.content ?? "NOVA"}</strong></button><button className={selectedId === headerNavigation?.id ? "canvas-node selected" : "canvas-node"} onClick={event => { event.stopPropagation(); if (headerNavigation) onSelect(headerNavigation.id); }} style={headerNavigation ? canvasStyle(headerNavigation) : undefined}><span>{headerNavigation?.content}</span></button><i /></div>{bottomDivider}</section>;
  if (node.id === "hero") return <section className={selected ? "manual-canvas-section selected canvas-hero" : "manual-canvas-section canvas-hero"} style={sectionCanvasStyle} onDragOver={event => event.preventDefault()} onDrop={() => onDropElement(node.id)} onClick={() => onSelect(node.id)}>{topDivider}<div className="canvas-hero-copy"><span className="canvas-kicker">NEW SEASON</span><button className={selectedId === heading?.id ? "canvas-node canvas-heading selected" : "canvas-node canvas-heading"} onClick={event => { event.stopPropagation(); if (heading) onSelect(heading.id); }} style={heading ? { ...canvasStyle(heading), ...(selectedId === heading.id ? textStyle : {}) } : undefined}>{(heading?.content ?? "Designed to feel\nlike your own.").split("\n").map(line => <span key={line}>{line}</span>)}</button><button className={selectedId === description?.id ? "canvas-node canvas-description selected" : "canvas-node canvas-description"} onClick={event => { event.stopPropagation(); if (description) onSelect(description.id); }} style={description ? { ...canvasStyle(description), ...(selectedId === description.id ? textStyle : {}) } : undefined}>{description?.content}</button><button className={selectedId === cta?.id ? "canvas-node canvas-cta selected" : "canvas-node canvas-cta"} onClick={event => { event.stopPropagation(); if (cta) onSelect(cta.id); }} style={cta ? canvasStyle(cta, true) : undefined}>{cta?.content}</button>{extraChildren.map(child => <button className={selectedId === child.id ? "canvas-inserted-node selected" : "canvas-inserted-node"} onClick={event => { event.stopPropagation(); onSelect(child.id); }} style={canvasStyle(child, child.kind === "shape")} key={child.id}>{child.kind === "shape" || child.kind === "vector" ? <i style={{ background: resolveStyle(child).background }} /> : <span>{child.content ?? child.name}</span>}</button>)}</div><button className={selectedId === "hero-image" ? "canvas-product-object selected" : "canvas-product-object"} onClick={event => { event.stopPropagation(); onSelect("hero-image"); }} style={node.style.animation !== "None" ? { animation: "manual-canvas-fade .2s ease-out" } : undefined}><div className="canvas-object-shadow" /><div className="canvas-object-shape" style={canvasStyle(nodesafe("hero-image", children, nestedChildren), true)} /><span>{nodesafe("hero-image", children, nestedChildren).content ?? "Selected image"}</span></button>{bottomDivider}</section>;
  if (node.id === "products") return <section className={selected ? "manual-canvas-section selected canvas-products" : "manual-canvas-section canvas-products"} style={sectionCanvasStyle} onDragOver={event => event.preventDefault()} onDrop={() => onDropElement(node.id)} onClick={() => onSelect(node.id)}>{topDivider}<button className={selectedId === "products-heading" ? "canvas-node canvas-products-heading selected" : "canvas-node canvas-products-heading"} onClick={event => { event.stopPropagation(); onSelect("products-heading"); }} style={canvasStyle(nodesafe("products-heading", children, nestedChildren))}>{nodesafe("products-heading", children, nestedChildren).content ?? "Made for the everyday"}</button><button className={selectedId === "product-grid" ? "canvas-product-grid selected" : "canvas-product-grid"} onClick={event => { event.stopPropagation(); onSelect("product-grid"); }} style={canvasStyle(nodesafe("product-grid", children, nestedChildren))}>{["Clay vessel", "Field towel", "Carry case"].map((item, index) => <span key={item}><i className={`product-tile tile-${index}`} /><b>{item}</b><small>${index === 0 ? "$48" : index === 1 ? "$28" : "$34"}</small></span>)}</button>{children.filter(item => !["products-heading", "product-grid"].includes(item.id) && item.visible).map(child => <button className={selectedId === child.id ? "canvas-inserted-node selected" : "canvas-inserted-node"} onClick={event => { event.stopPropagation(); onSelect(child.id); }} style={canvasStyle(child, child.kind === "shape")} key={child.id}>{child.kind === "shape" || child.kind === "vector" ? <i style={{ background: resolveStyle(child).background }} /> : <span>{child.content ?? child.name}</span>}</button>)}{bottomDivider}</section>;
  return <section className={selected ? "manual-canvas-section selected canvas-footer" : "manual-canvas-section canvas-footer"} style={sectionCanvasStyle} onDragOver={event => event.preventDefault()} onDrop={() => onDropElement(node.id)} onClick={() => onSelect(node.id)}>{topDivider}<strong>NOVA</strong><span>Made for a slower everyday.</span>{bottomDivider}</section>;
}

function nodesafe(id: string, children: EditorNode[], nestedChildren: (id: string) => EditorNode[]) {
  return children.find(item => item.id === id) ?? nestedChildren("hero-copy").find(item => item.id === id) ?? { id, parentId: null, name: id, kind: "text" as NodeKind, visible: true, locked: false, style: {} };
}

function SectionDivider({ edge, shape, height, color }: { edge: "top" | "bottom"; shape: Exclude<SectionDividerShape, "none">; height: number; color: string }) {
  const paths: Record<Exclude<SectionDividerShape, "none">, string> = {
    wave: "M0 12 C12 2 25 2 37 12 S62 22 75 12 S88 2 100 12 V24 H0 Z",
    curve: "M0 0 C25 23 75 23 100 0 V24 H0 Z",
    angle: "M0 0 L100 18 V24 H0 Z",
    "u-shape": "M0 0 C18 0 22 20 50 20 S82 0 100 0 V24 H0 Z",
    "w-shape": "M0 0 L25 18 L50 3 L75 18 L100 0 V24 H0 Z",
    "custom-vector": "M0 8 L20 1 L42 17 L62 4 L79 20 L100 7 V24 H0 Z",
  };
  return <span className={`manual-section-divider ${edge}`} style={{ height }} aria-label={`${shape} ${edge} divider`}><svg viewBox="0 0 100 24" preserveAspectRatio="none"><path d={paths[shape]} fill={color} /></svg></span>;
}
