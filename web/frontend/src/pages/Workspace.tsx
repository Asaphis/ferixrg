...
const canonicalStorePanelTools = [
  { id: "inspect", label: "Inspect", title: "Inspect visible evidence", description: "Review uploaded screenshots or persisted analysis evidence. An executor is required before a result is created.", toolId: "screenshot-analyzer", sources: ["Screenshots", "Analysis result"] as const, requiresConnection: false },
  { id: "analyse", label: "Analyse", title: "Analyse this store", description: "Run the supported storefront analyzer against a public URL or a persisted store source.", toolId: "storefront-analyzer", sources: ["Public URL", "Connected store"] as const, requiresConnection: false },
  { id: "editor", label: "Editor", title: "Open the visual editor", description: "Open a saved draft only. No live storefront preview is fabricated when no rendered artifact exists.", route: "Visual editor", sources: ["Saved draft", "Analysis result"] as const, requiresConnection: false },
  { id: "issues", label: "Issues", title: "Review recorded issues", description: "Open issue records created by a completed executor run.", route: "Issues", sources: ["Analysis result"] as const, requiresConnection: false },
  { id: "validate", label: "Validate", title: "Validate release readiness", description: "Run integrity validation against a persisted saved draft version.", toolId: "publish-readiness-checker", sources: ["Saved draft", "Connected store"] as const, requiresConnection: true },
  { id: "publish", label: "Publish", title: "Prepare a supported store update", description: "Create a release plan for a supported connected store. Publishing requires configured provider execution.", toolId: "publish-manager", sources: ["Connected store"] as const, requiresConnection: true },
] as const;
...
export default function Workspace() {
  ...
  const [selectedIssue, setSelectedIssue] = useState<{ title: string }>({ title: "" });
  ...
  function ToolsLibrary() {
    ...
    if (toolFlow !== "library") {
      ...
      return <ApprovedToolWorkflow key={`${selectedTool.id}-${toolFlow}`} tool={selectedTool} workspaceId={activeWorkspaceId} storeId={activeStore?.id} startAt={startAt} startSource={selectedToolSource ?? requestedToolSource ?? undefined} onBack={() => setToolFlow("library")} />;
    }
  ...
  function StorePanel() {
    ...
    if (!store) return <section className="mobile-flow-page"><FlowHeader back={() => changeView("Stores")} title="No live store selected" copy="This workspace panel only shows stores returned by the authenticated workspace API." /><section className="flow-card"><b>No persisted store record is available.</b><p>Add a public URL or complete a supported connection before opening a store workspace. No sample store data is shown.</p></section><button className="flow-primary" onClick={() => changeView("Stores")}>Back to Stores</button></section>;
    const storeActivity: Array<{ id: string; title: string; description: string; time: string; route: string; toolId?: string }> = [];
    const [selectedPanelToolId, setSelectedPanelToolId] = useState("inspect");
    const selectedPanelTool = canonicalStorePanelTools.find(tool => tool.id === selectedPanelToolId) ?? canonicalStorePanelTools[0];
  ...
  function Placeholder({ title, copy, icon, action }: { title: string; copy: string; icon: React.ReactNode; action: string }) { return <><PageHeading label={`FerixRG workspace${workspaceStoresQuery.data?.[0]?.name ? ` / ${workspaceStoresQuery.data[0].name}` : ""}`} title={title} copy={copy} /><section className="placeholder-page"><div><div className="empty-icon">{icon}</div><h2>{title}</h2><p>{copy}</p><button className="app-button" onClick={() => changeView("Analysis")}><ArrowRight /> {action}</button></div></section></> }
}
