...
export default function Workspace() {
  ...
  const beginUrlAnalysis = async () => {
    ...
    changeView("Stores");
    toast.success("URL inspection is ready", { description: "Observed storefront evidence and a JSON inspection export are available in the Stores workspace." });
  };
  ...
  function ToolsLibrary() {
    ...
    const chooseStart = (choice: string) => { setStartChoice(choice); setSelectedToolSource(choice); setLaunchNote(""); };
  ...
  function LegacyOverview() {
    ...
    const overviewStoreId = sidebarStore ? String(sidebarStore.id) : activeStore.id;
    return <section className="approved-dashboard">
      <header className="approved-greeting"><div><span className="approved-eyebrow">Your storefront command center</span><h1>Good morning, <b>{accountDisplayName}.</b></h1><p>{approvedDashboard.greeting.copy}</p></div><div className="approved-greeting-actions"><button className="approved-primary" onClick={() => openTool("storefront-analyzer")}><ScanLine /> Analyze a store</button><button className="approved-secondary" onClick={() => openTool("publish-manager")}><Plus /> Connect store</button></div></header>
      ...
      <section className="approved-section-heading"><div><span className="approved-eyebrow">Store overview</span><h2>Your Stores</h2></div><button onClick={() => openStore(overviewStoreId)}>View all <ChevronRight /></button></section>
  ...
  function StorePanel() {
    ...
    } : connectedStores.find(item => item.id === activeStoreId) ?? connectedStores[0];
    const storeActivity = liveStore ? [] : storeActivities.filter(activity => activity.storeId === store.id);
  ...
  function Issues() {
    ...
    const displayedIssue = filtered.find(issue => issue.title === selectedIssue.title) ?? filtered[0];
    return <>
