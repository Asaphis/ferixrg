...
export default function Workspace() {
  ...
  const beginStoreConnection = async () => {
    ...
    try {
      ...
      let connectionStore = workspaceStoresQuery.data?.find(store => String(store.id) === activeStoreId && store.platform === "shopify") ?? workspaceStoresQuery.data?.find(store => store.platform === "shopify");
      if (!connectionStore) {
        const parsed = new URL(storeUrl);
        if (!/^https?:$/.test(parsed.protocol)) throw new Error("Enter the public HTTPS storefront URL before connecting Shopify.");
        const created = await createStoreMutation.mutateAsync({ workspaceId: activeWorkspaceId, name: parsed.hostname.replace(/^www\./, "") || "Shopify storefront", platform: "shopify", url: parsed.toString() });
  ...
  function ToolsLibrary() {
    ...
    if (toolFlow !== "library") {
      ...
      return <ApprovedToolWorkflow key={`${selectedTool.id}-${toolFlow}`} tool={selectedTool} workspaceId={activeWorkspaceId} startAt={startAt} startSource={requestedToolSource ?? undefined} onBack={() => setToolFlow("library")} />;
    }
