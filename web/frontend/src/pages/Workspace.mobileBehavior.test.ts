...
const sessionMocks = vi.hoisted(() => ({
  ...
  executeReleaseAction: vi.fn().mockResolvedValue({ action: { id: 91, status: "published" }, providerReference: "provider-ref-91" }),
  ...
  layoutComposer: vi.fn().mockResolvedValue({ response: "Proposal: Place the product proof beside the primary action and retain manual layout control. Review before applying.", model: "@cf/meta/llama-3.2-3b-instruct", neurons: 1 }),
  componentBuilder: vi.fn().mockResolvedValue({ response: "Proposal: Use a reusable product-benefit card with manual component controls. Review before applying.", model: "@cf/meta/llama-3.2-3b-instruct", neurons: 1 }),
...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    ...
    workspace: {
      ...
      toolRuns: { useQuery: () => ({ data: sessionMocks.toolRuns, isLoading: false }) },
      issues: { useQuery: () => ({ data: sessionMocks.issues, isLoading: false }) },
      reports: { useQuery: () => ({ data: sessionMocks.reports, isLoading: false }) },
      ...
      executeReleaseAction: { useMutation: () => ({ mutateAsync: sessionMocks.executeReleaseAction }) },
      ...
      layoutComposer: { useMutation: () => ({ mutateAsync: sessionMocks.layoutComposer }) },
      componentBuilder: { useMutation: () => ({ mutateAsync: sessionMocks.componentBuilder }) },
