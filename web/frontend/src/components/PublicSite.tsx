...
export function HeroMontage() {
  ...
  return <div className="hero-montage" aria-label="Animated storefront intelligence preview" onMouseMove={move} onMouseLeave={reset}><div className="montage-workspace" data-motion-layer><img src={heroAsset} alt="FerixRG storefront analysis workspace" /><span>STORE HEALTH · 82 / 100</span></div><div className="montage-browser" data-motion-layer><div className="browser-chrome"><i /><i /><i /><span>YOURSTORE · Shop · Collections</span></div><div className="browser-store"><b>Timeless living.<br />thoughtfully curated.</b><small>Shop the collection</small></div><div className="browser-products"><i /><i /><i /></div></div><div className="montage-phone" data-motion-layer><img src={mobileAsset} alt="Allbirds mobile storefront phone preview" /></div><div className="montage-connector" data-motion-layer /><div className="montage-insight" data-motion-layer><div className="landing-eyebrow"><Sparkles size={10} /> AI INSIGHT</div><strong>91 <small>/100</small></strong><p>Your store is strong. We found 18 opportunities to improve.</p></div><div className="montage-impact" data-motion-layer><span>REDESIGN IMPACT</span><b>72 <em>→</em> 94</b><small>Before &nbsp;&nbsp;&nbsp;&nbsp; After</small></div><div className="montage-badge" data-motion-layer><Sparkles size={13} /> AI redesign ready</div></div>;
}

export function UrlCapture({ onAnalyze }: { onAnalyze: () => void }) {
  const [url, setUrl] = useState("");
  return <form className="landing-url" onSubmit={event => { event.preventDefault(); onAnalyze(); }}><label>Paste your store URL</label><div><Link2 size={14} /><input aria-label="Store URL" value={url} onChange={event => setUrl(event.target.value)} placeholder="https://yourstore.com" /><button type="submit">Analyze Store</button></div><small>Analyze any public storefront without connecting your store.</small></form>;
}
