...
import { filterTools, toolCatalog, toolCategories, type ToolDefinition } from "@/lib/toolCatalog";
import { connectedStores, storeActivities, storeQuickActions } from "@/lib/storeWorkspace";
...
export default function Workspace() {
  ...
  const openTool = (toolId: string) => { setToolIntent(toolId); changeView("Tools Library"); };
  ...
  return <div className="workspace">
    ...
    <main className={`app-main ${view === "Overview" || view === "Store workspace" ? "overview-mode" : ""}`}><header className={`app-topbar ${view === "Overview" ? "command-topbar" : ""}`}>{view === "Overview" ? <><div className="command-shell-context"><div className="store-dot" /><span>Atelier Forma</span><small>Store workspace</small></div><div className="command-top-actions"><button className="command-search" onClick={() => changeView("Tools Library")}><Search /> Find a tool or workflow <kbd>⌘ K</kbd></button><button className="command-alert" onClick={() => changeView("Issues")} aria-label="Open issue alerts"><Bell /><i /></button><button className="command-avatar" onClick={() => changeView("Overview")}>AT</button></div></> : <><div className="top-context"><div className="store-dot" /><div><span className="crumb">Stores / Atelier Forma /</span><strong>{view}</strong></div></div><div className="top-actions"><button className="search-trigger" onClick={() => changeView("Analysis")}><Search size={14}/> Search anything <kbd>⌘ K</kbd></button><button className="round-icon" onClick={() => changeView("Issues")} aria-label="Open notifications"><Bell /></button><button className="app-button" onClick={() => {setScanRunning(true); changeView("Analysis");}}><RefreshCw size={14} className={scanRunning ? "animate-spin" : ""}/><span>{scanRunning ? "Scanning" : "Re-scan"}</span></button></div></>}</header><div className="app-content">{renderView()}</div></main>
  ...
  function ToolsLibrary() {
    ...
    const categoryCount = (item: (typeof toolCategories)[number]) => filterTools(query, item).length;
    ...
    return <>
      ...
      <section className="library-discovery-bar" aria-label="Search the Tools Library"><div className="library-query-field"><Search size={18}/><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search by tool, function, input, or outcome — e.g. redesign, screenshot, publish, GitHub" /><kbd>⌘ K</kbd>{query && <button onClick={() => setQuery("")} aria-label="Clear tool search">Clear</button>}</div><p>{hasDiscoveryFilter ? <><b>{visibleTools.length}</b> matching tools</> : <>Search across <b>23 tools</b>, their functions, accepted inputs, and connection requirements.</>}</p></section>
      <section className="tool-library-shell">
        <aside className="tool-library-rail">
          <span className="card-eyebrow" style={{color:'#155eef'}}>Filter the catalogue</span>
          <div className="tool-category-list">{toolCategories.map(item => <button className={category === item ? "active" : ""} onClick={() => setCategory(item)} key={item}><span>{item}</span><b>{categoryCount(item)}</b></button>)}</div>
          {hasDiscoveryFilter && <button className="clear-discovery-button" onClick={clearDiscovery}>Clear all filters</button>}
        ...
        <section className="tool-results"><div className="tool-results-heading"><div><span className="card-eyebrow" style={{color:'#155eef'}}>{category}</span><h2>{visibleTools.length} {visibleTools.length === 1 ? "tool" : "tools"}, with the setup made visible.</h2></div><p>Inspect freely with URLs or screenshots. Connect only when the selected tool needs private data, theme access, or permission to prepare a store change.</p></div><div className="category-chip-row" aria-label="Tool categories">{toolCategories.map(item => <button className={category === item ? "active" : ""} onClick={() => setCategory(item)} key={item}>{item}<b>{categoryCount(item)}</b></button>)}{hasDiscoveryFilter && <button className="clear" onClick={clearDiscovery}>Reset</button>}</div><div className="tool-card-grid">{visibleTools.length ? visibleTools.map(tool => { const ToolIcon = iconFor(tool); return <button className={`tool-library-card ${selectedTool.id === tool.id ? "selected" : ""}`} onClick={() => selectTool(tool.id)} key={tool.id}><div className="tool-card-top"><span className={`tool-icon ${tool.category.toLowerCase().replaceAll(" ", "-").replace("&", "and")}`}><ToolIcon /></span><span className={tool.requiresConnection ? "connection-badge required" : "connection-badge open"}>{tool.requiresConnection ? "Connection needed" : "No connection needed"}</span></div><h3>{tool.name}</h3><p>{tool.description}</p><div className="tool-card-inputs">{tool.sources.slice(0,3).map(source => <span key={source}>{source}</span>)}</div></button> }) : <div className="tool-empty-state"><Search /><h3>No matching tools yet.</h3><p>Try a capability such as “redesign,” an input like “screenshot,” or a connection such as “Shopify” or “GitHub.”</p><button className="app-button" onClick={clearDiscovery}>Clear search and filters</button></div>}</div></section>
  ...
  function Overview() {
    const iconFor = (icon: string) => icon === "scan" ? <ScanLine /> : icon === "evidence" ? <Eye /> : icon === "draft" ? <Layers3 /> : <Bot />;
    const openAction = (action: typeof storeQuickActions[number]) => action.toolId ? openTool(action.toolId) : changeView(action.route ?? "Visual editor");
    const openActivity = (activity: typeof storeActivities[number]) => activity.toolId ? openTool(activity.toolId) : changeView(activity.route);
    return <section className="simple-workspace">
      <header className="simple-workspace-header"><div><span className="command-kicker">Your store workspace</span><h1>Welcome back, <b>Atelier Forma.</b></h1><p>Choose a quick action, pick a connected store, or continue work already in progress.</p></div><button className="simple-connect-button" onClick={() => openTool("store-publisher")}><Plus /> Connect a store</button></header>
      <section className="simple-section"><div className="simple-section-title"><div><span>Quick actions</span><h2>Start with what you need to do</h2></div><p>Each action takes you directly to the right tool. You will see connection requirements before anything starts.</p></div><div className="simple-action-grid">{storeQuickActions.map(action => <button onClick={() => openAction(action)} key={action.id}><span className={`simple-action-icon ${action.icon}`}>{iconFor(action.icon)}</span><span><b>{action.title}</b><p>{action.description}</p></span><ChevronRight /></button>)}</div></section>
      <section className="simple-workspace-grid"><article className="stores-card"><div className="simple-card-heading"><div><span>Connected stores</span><h2>Choose a store to work on</h2><p>Every store has its own panel for analysis, issues, drafts, validation, and publishing work.</p></div><button onClick={() => openTool("store-publisher")}>Add store <Plus /></button></div><div className="store-list">{connectedStores.map(store => <button className={`store-list-item ${store.connection === "Needs attention" ? "attention" : ""}`} onClick={() => openStore(store.id)} key={store.id}><span className="store-initials">{store.initials}</span><span className="store-list-main"><b>{store.name}</b><small>{store.platform} · {store.url}</small><em>{store.lastActivity}</em></span><span className="store-list-status"><small className={store.connection === "Connected" ? "connected" : "attention"}><i /> {store.connection}</small><span><b>{store.openIssues}</b> issues · <b>{store.drafts}</b> drafts</span></span><ChevronRight /></button>)}</div></article><aside className="activity-card-simple"><div className="simple-card-heading"><div><span>Recent activity</span><h2>Work in progress</h2><p>Open any record to continue from the right place.</p></div></div><div className="activity-list-simple">{storeActivities.map((activity, index) => <button onClick={() => openActivity(activity)} key={activity.id}><span className={`activity-status-dot ${activity.status.toLowerCase().replace(" ", "-")}`}><i /></span><span><b>{activity.title}</b><p>{activity.description}</p><small>{connectedStores.find(store => store.id === activity.storeId)?.name} · {activity.time}</small></span><span className="activity-open">Open <ChevronRight /></span></button>)}</div></aside></section>
    </section>;
  }
