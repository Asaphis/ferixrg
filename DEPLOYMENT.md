...
|---|---|---|---|
| `DATABASE_URL` | **Secret** | Pooled Neon PostgreSQL connection string used by the runtime application. | Required. Use the pooled connection string for normal requests. |
| `DATABASE_URL_UNPOOLED` | **Secret** | Direct Neon PostgreSQL connection string used by Drizzle migrations. | Required for `pnpm drizzle-kit migrate`; keep it separate from the pooled runtime URL. |
